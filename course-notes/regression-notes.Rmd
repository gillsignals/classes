...
```{r}
-2.769 + 0.371 *1 + 0.519 *6 + .771 *2 + 1.24 * 1 + 1.43 * 0
```
...
players %>%
  ...
  scale_x_log10()
```
We can search for good deals by looking at players that produce many more runs than others with similar salaries. We can use this table to decide what players to pick and keep our total salary below $40 million. This can be done with _linear programming_, which we do not teach but include the code anyway:
```{r}
library(reshape2)
library(lpSolve)

players <- players %>%
  filter(debut <= 1997 & debut > 1988)
...
lp_solution <- lp("max", players$R_hat, constraint_matrix, constraint_dir, constraint_limit, all.int = TRUE)
```
Our algorithm chooses these 9 players:
```{r}
our_team <- players %>%
...
our_team %>%
  select(nameFirst, nameLast, POS, salary, R_hat)
```
Our team has players above average on BB and HR, but not singles.
...
players %>% mutate(BB = my_scale(BB),
  ...
  arrange(desc(R_hat))
```
Sabermetricians have used a metric beyond batting average that they consider better because it includes walks and weights base hits by number of bases - on base plus slugging (OPS):

$$\frac{\mbox{BB}}{\mbox{PA}} + \frac{\mbox{Singles} + 2 \mbox{Doubles} + 3 \mbox{Triples} + 4\mbox{HR}}{\mbox{AB}}$$
This is impressively close to what one gets with regression:
```{r}
Batting %>%
